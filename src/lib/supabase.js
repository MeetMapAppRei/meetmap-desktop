...
const mapCreateEventError = (error) => {
  ...
  if (
    code === "23505" ||
    /events_title_date_city_unique/i.test(message) ||
    /events_title_date_city_unique/i.test(details)
  ) {
...
const fetchEventRsvpStatsMap = async (eventIds) => {
  ...
  for (const row of data || []) {
    if (!map[row.event_id])
      map[row.event_id] = { interested_count: 0, going_count: 0 };
...
export const updateEvent = async (eventId, updates) => {
  ...
  try {
    ...
    rsvpStats = {
      interested_count: 0,
      going_count: data.event_attendees?.[0]?.count || 0,
    };
  }
  return {
    ...data,
    status: finalStatus,
    status_note: finalStatusNote,
    ...latest,
    ...rsvpStats,
  };
};
...
export const createFlyerImport = async ({
  userId,
  sourceUrl,
  imageUrl,
  extracted,
  ...
  const tagsArray =
    extracted?.tags && Array.isArray(extracted.tags)
      ? extracted.tags
      : extracted?.tags || [];
  const payload = {
    user_id: userId,
...
export const fetchDailyFlyerImportReview = async (date = new Date()) => {
  ...
  const { data: candidates, error: candidatesError } = await supabase
    ...
    .select(
      "*, events(id, title, date, city, address, location, photo_url, created_at)",
    )
    .in("run_id", runIds)
    .order("created_at", { ascending: false });
...
async function uploadImageViaR2Presign(file, body) {
  ...
  if (!json?.uploadUrl) {
    throw (
      lastPresignErr || new Error("Could not reach upload service. Try again.")
    );
  }
  ...
  try {
    ...
    throw lastRelayErr instanceof Error
      ? lastRelayErr
      : new Error(String(lastRelayErr));
  }
  return publicUrl;
}
...
export const uploadEventPhoto = async (file, eventId) => {
  ...
  const contentType =
    ready.type || `image/${safeExt === "jpg" ? "jpeg" : safeExt}`;
...
export const getEventRsvpStatus = async (eventId, userId) => {
  ...
  try {
    ...
    const attending = await getAttendanceStatus(eventId, userId).catch(
      () => false,
    );
