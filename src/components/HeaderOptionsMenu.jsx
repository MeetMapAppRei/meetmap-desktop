...
function MenuToggle({ label, active, onClick, isLight, panelBorder }) {
  return (
    ...
    >
      <span style={{ width: 16, color: active ? '#FF6B35' : panelBorder }}>{active ? '✓' : '○'}</span>
      {label}
    </button>
  )
}
  ...
  const menuPanel = open && menuPos && (
    <div
      ...
      style={{
        ...
        borderRadius: 10,
        boxShadow: isLight ? '0 8px 24px rgba(0,0,0,0.12)' : '0 8px 24px rgba(0,0,0,0.45)',
        zIndex: 10000,
        overflow: 'hidden',
        paddingBottom: 4,
      }}
    >
      ...
      <div style={{ height: 1, background: panelBorder, margin: '6px 0 4px' }} />
      ...
      />
      <MenuToggle
        label={showSavedOnly && savedCount > 0 ? `Saved only (${savedCount})` : 'Saved only'}
